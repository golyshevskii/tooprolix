...
//! produces links libpython at all and there is nothing left to point an rpath at.
//! `pyo3-build-config` went from `[build-dependencies]` in the same change.
//!
//! **The file itself stays, and deleting it would be a silent regression**: the resolver below is
//! the ONLY source of `TOOPROLIX_COMMIT_DATE`, and without it every published artifact reports
//! `unknown` as its build date.
//!
